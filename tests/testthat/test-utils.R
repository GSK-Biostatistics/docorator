...
test_that("markdown chunks are created correctly", {
  ...
  chunk_png <- create_chunks_all(x = docorator, transform = NULL) |>
    capture.output()
  ...
  my_gt <- gt::exibble |>
    ...
    apply_scale(fontsize = 10, tbl_scale = TRUE, tbl_stub_pct = 0.5)
  docorator <- as_docorator(my_gt, display_name = "mytbl", save_object = FALSE)
  chunk_gt <- create_chunks_all(x = docorator, transform = NULL) |>
    capture.output()
  ...
  chunk_gt_group <- create_chunks_all(x = docorator, transform = NULL) |>
    capture.output()
  expect_snapshot(chunk_gt_group)

  my_gt_group_caption <- gt::gt_group(
    my_gt |> gt::tab_caption("test"),
    my_gt |> gt::tab_caption("test")
  )
  docorator <- as_docorator(
    my_gt_group_caption,
  ...
  chunk_gt_group_caption <- create_chunks_all(
    x = docorator,
    transform = NULL
  ) |>
    capture.output()
  ...
  withr::with_tempdir({
    ...
    docorator <- as_docorator(
      list_of_ggplots,
      display_name = "my_plot_list",
      save_object = FALSE
    )
    chunk_list <- create_chunks_all(x = docorator, transform = NULL) |>
      capture.output()
...
test_that("package version messages are printed correctly", {
  ...
  suppressMessages({
    expected_message <- cli::cli_text(
      "Note: docorator object was created with {.pkg gt 0.0.1234}. You are now running {.pkg gt {current_version}}. There may be issues rendering your document."
    )
