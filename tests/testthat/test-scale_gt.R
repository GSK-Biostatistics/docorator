test_that("scale_gt works", {
  ...
  expect_equal(
    dplyr::filter(gt_out, !type == "stub" & var == "group")$column_width |>
      unlist(),
    "15%"
  )
  expect_equal(
  ...
  expect_equal(unique(widths), 100 / ncol(gt::exibble))

  # table with 1 stub column that is a groupname_col (not rowname_col)
  tbl_grp1stub <- gt::exibble |>
    gt::gt(groupname_col = "fctr", row_group_as_column = TRUE)

  gt_out <- scale_gt(tbl_grp1stub, tbl_stub_pct = 0.3)$`_boxhead`

  expect_equal(dplyr::filter(gt_out, type == "stub") |> nrow(), 0)
  expect_equal(
...
test_that("scale_gt works with gt >= 1.1.0", {
  ...
  expect_equal(
    dplyr::filter(gt_out, !type %in% c("stub", "row_group"))$column_width |>
      unlist(),
    rep("10%", ncol(gt::exibble) - 2)
  )
...
test_that("check_gt_widths works", {
  ...
  expect_equal(
    check_gt_widths(no_widths),
    list(tbl_scale = FALSE, table_width = "100%")
  )
  ...
  expect_warning(
    check_gt_widths(large_widths),
    "Column widths must be add to =<100%, not 403.5%. Applying auto table scaling."
  )
  expect_equal(
    suppressWarnings(check_gt_widths(large_widths)),
    list(tbl_scale = TRUE, table_width = "100%")
  )
  ...
  expect_warning(
    check_gt_widths(px_widths2),
    "Column widths must be specified in % not px. Applying auto table scaling."
  )
  expect_equal(
    suppressWarnings(check_gt_widths(px_widths2)),
    list(tbl_scale = TRUE, table_width = "100%")
  )
  ...
  expect_equal(
    check_gt_widths(small_widths),
    list(tbl_scale = FALSE, table_width = NULL)
  )
})
...
test_that("apply_scale works - gt_tbl", {
  ...
  expect_warning(
    apply_scale(gt_tbl1, fontsize = 10, tbl_scale = FALSE, tbl_stub_pct = 0.3),
  ...
  expect_identical(
    ...
    scale_gt(gt_tbl1, tbl_stub_pct = 0.3)
  )

  # col widths <100
  gt_tbl2 <- gt::exibble |>
    gt::gt() |>
    gt::cols_width(num ~ "10%", char ~ "3%")

  expect_message(
    apply_scale(gt_tbl2, fontsize = 10, tbl_scale = FALSE, tbl_stub_pct = 0.3),
    "NOTE: Column widths total <100%. Resulting table may not respect document margins."
  )
  expect_identical(
    suppressMessages(apply_scale(
      gt_tbl2,
      fontsize = 10,
      tbl_scale = FALSE,
      tbl_stub_pct = 0.3
  ...
  gt_tbl3 <- gt::exibble |>
    ...
    gt::gt() |>
    gt::cols_width(everything() ~ "50%")

  expect_identical(
    apply_scale(gt_tbl3, fontsize = 10, tbl_scale = FALSE, tbl_stub_pct = 0.3),
    gt_tbl3 |>
      gt::tab_options(
        table.font.size = "10pt",
        heading.subtitle.font.size = "10pt",
  ...
  expect_identical(
    apply_scale(gt_tbl4, fontsize = 10, tbl_scale = TRUE, tbl_stub_pct = 0.3),
    scale_gt(gt_tbl4, tbl_stub_pct = 0.3) |>
      gt::tab_options(
        table.font.size = "10pt",
        heading.subtitle.font.size = "10pt",
        heading.title.font.size = "10pt",
...
test_that("check_gt_widths works - hidden columns", {
  ...
  expect_message(
    check_gt_widths(hidden_cols),
    "NOTE: Column widths total <100%. Resulting table may not respect document margins."
  )
  expect_equal(
    suppressMessages(check_gt_widths(hidden_cols)),
    list(tbl_scale = FALSE, table_width = "100%")
  )
})
...
test_that("apply_scale works - gt_group", {
  ...
  expected_group2 <- gt::gt_group(
    scale_gt(gt_tbl, tbl_stub_pct = 0.3),
    scale_gt(gt_tbl, tbl_stub_pct = 0.3)
  )

  expect_identical(
    apply_scale(gt_group2, fontsize = 10, tbl_scale = TRUE, tbl_stub_pct = 0.3),
    expected_group2
  )
})
